...
contract ConvertToPermanent_StakeWeight_Unit_Fuzz_Test is Integration_Test {
    ...
    function testFuzz_ConvertToPermanent_DurationValidation(
        uint256 remainingWeeks,
        uint256 permanentWeeks
    ) public {
    ...
    function testFuzz_ConvertToPermanent_ExpiredLockHandling(uint256 lockDuration, uint256 timeAfterExpiry) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                StakeWeight.ExpiredLock.selector,
                block.timestamp,
                actualLockEnd
            )
        );
    ...
    function testFuzz_ConvertToPermanent_WeightCalculationAccuracy(
        uint256 amount,
