...
contract MixedPositions_StakingRewardDistributor_Integration_Concrete_Test is StakeWeight_Integration_Shared_Test {
    ...
    function test_MixedPositions_RapidConversions() external {
        ...
        address payable[5] memory users_ =
            [users.alice, users.bob, users.carol, payable(makeAddr("user4")), payable(makeAddr("user5"))];
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            assertApproxEqRel(rewards, weeklyAmount * 4 / 5, 0.01e18, "Should get ~1/5 of 4 weeks rewards");
        }

        // Total claimed should be approximately 4 weeks of rewards
        assertApproxEqRel(totalClaimed, weeklyAmount * 4, 0.01e18, "Total claimed should be ~4 weeks of rewards");
    }
