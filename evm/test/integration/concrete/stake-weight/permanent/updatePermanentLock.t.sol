...
contract UpdatePerpetualLock_StakeWeight_Integration_Concrete_Test is StakeWeight_Integration_Shared_Test {
    ...
    function test_RevertWhen_NewDurationIsInvalid() external whenContractIsNotPaused whenLockIsPerpetual {
        deal(address(l2wct), users.alice, ADDITIONAL_AMOUNT);
        vm.startPrank(users.alice);
        IERC20(address(l2wct)).approve(address(stakeWeight), ADDITIONAL_AMOUNT);
    ...
    function test_RevertWhen_NewDurationIsShorterThanCurrent() external whenContractIsNotPaused whenLockIsPerpetual {
    ...
    function test_WhenOnlyAddingAmount() external whenContractIsNotPaused whenLockIsPerpetual whenNewDurationIsValid {
        ...
        assertEq(
            stakeWeight.permanentBaseWeeks(users.alice) * 1 weeks, INITIAL_DURATION, "Duration should remain unchanged"
        );
        ...
        whenNewDurationIsValid
    {
        ...
        StakeWeight.LockedBalance memory lockAfter = stakeWeight.locks(users.alice);
        assertEq(
            stakeWeight.permanentBaseWeeks(users.alice) * 1 weeks, newDuration, "Duration should be updated to 26 weeks"
        );
        assertEq(uint256(int256(lockAfter.amount)), expectedNewAmount, "Amount should be sum of old and new");
        assertEq(lockAfter.end, 0, "Should still be permanent (end = 0)");
    ...
    function test_MultipleUpdates() external whenContractIsNotPaused whenLockIsPerpetual whenNewDurationIsValid {
        vm.startPrank(users.alice);
        ...
        assertEq(
            stakeWeight.permanentBaseWeeks(users.alice) * 1 weeks, INITIAL_DURATION, "Duration should still be 4 weeks"
        );
