    ...
    IERC7802
{
    ...
    error InvalidBridgeZeroAddress();
    ...
    function initialize(Init memory init) public initializer {
        ...
        if (init.initialMinter == address(0)) revert InvalidMinterZeroAddress();
    ...
    function setBridge(address newBridge) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (newBridge == address(0)) {
            revert InvalidBridgeZeroAddress();
        }
    ...
    function _burn(address from, uint256 amount) internal override(NttTokenUpgradeable, ERC20Upgradeable) {
        super._burn(from, amount);
    }

    function _mint(address to, uint256 amount) internal override(NttTokenUpgradeable, ERC20Upgradeable) {
