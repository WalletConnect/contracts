...
// lib/example-native-token-transfers/evm/src/interfaces/ISpecialRelayer.sol
...
// lib/example-native-token-transfers/evm/src/libraries/BooleanFlag.sol
...
uint16 constant CHAIN_ID_SEPOLIA = 10_002;
uint16 constant CHAIN_ID_ARBITRUM_SEPOLIA = 10_003;
uint16 constant CHAIN_ID_BASE_SEPOLIA = 10_004;
uint16 constant CHAIN_ID_OPTIMISM_SEPOLIA = 10_005;
...
// lib/example-native-token-transfers/evm/src/interfaces/IWormholeTransceiverState.sol
...
// lib/example-native-token-transfers/evm/src/interfaces/IWormholeTransceiver.sol
...
interface IWormholeTransceiver is IWormholeTransceiverState {
    ...
    event SendTransceiverMessage(uint16 recipientChain, TransceiverStructs.TransceiverMessage message);
...
abstract contract CCTPSender is CCTPBase {
    ...
    function setCCTPDomain(uint16 chain, uint32 cctpDomain) public {
        require(msg.sender == cctpConfigurationOwner, "Not allowed to set CCTP Domain");
        ...
        returns (MessageKey memory)
    {
        ...
        uint64 nonce = circleTokenMessenger.depositForBurnWithCaller(
            amount, getCCTPDomain(targetChain), targetAddressBytes32, USDC, targetAddressBytes32
        );
...
abstract contract CCTPAndTokenBase is CCTPBase {
    ...
    constructor(
        ...
        CCTPBase(_wormholeRelayer, _wormhole, _circleMessageTransmitter, _circleTokenMessenger, _USDC)
    {
...
abstract contract CCTPAndTokenSender is CCTPAndTokenBase {
     ...
     *
     * Note: this function uses transferTokensWithPayload instead of transferTokens since the former requires that only
     * the targetAddress
     *       can redeem transfers. Otherwise it's possible for another address to redeem the transfer before the
     * targetContract is invoked by
        ...
        returns (uint64)
    {
        ...
        return wormholeRelayer.sendVaasToEvm{ value: cost }(
            targetChain, targetAddress, abi.encode(Transfer.TOKEN_BRIDGE, payload), receiverValue, gasLimit, vaaKeys
        );
    }
...
abstract contract CCTPAndTokenReceiver is CCTPAndTokenBase {
    ...
    struct TokenReceived {
        ...
        address tokenAddress; // wrapped address if tokenHomeChain !== this chain, else tokenHomeAddress (in evm address
            // format)
        uint256 amount;
        ...
        payable
    {
        ...
        if (transferType == Transfer.TOKEN_BRIDGE) {
            TokenReceived[] memory receivedTokens = new TokenReceived[](additionalMessages.length);

            for (uint256 i = 0; i < additionalMessages.length; ++i) {
                IWormhole.VM memory parsed = wormhole.parseVM(additionalMessages[i]);
                require(
            ...
            (, bytes memory userPayload) = abi.decode(payload, (Transfer, bytes));

            // call into overriden method
            receivePayloadAndTokens(userPayload, receivedTokens, sourceAddress, sourceChain, deliveryHash);
            ...
            (, uint256 amount, bytes memory userPayload) = abi.decode(payload, (Transfer, uint256, bytes));
...
// lib/example-native-token-transfers/evm/src/Transceiver/Transceiver.sol
...
abstract contract Transceiver is ITransceiver, PausableOwnable, ReentrancyGuardUpgradeable, Implementation {
        ...
        virtual
    {
        if (recipientNttManagerAddress != toWormholeFormat(nttManager)) {
            revert UnexpectedRecipientNttManagerAddress(toWormholeFormat(nttManager), recipientNttManagerAddress);
        }
...
// lib/example-native-token-transfers/evm/src/Transceiver/WormholeTransceiver/WormholeTransceiverState.sol
...
abstract contract WormholeTransceiverState is IWormholeTransceiverState, Transceiver {
    ...
    ///      This is 0x99'E''W''H'
    /// @notice Magic string (constant value set by messaging provider) that idenfies the payload as an
    /// transceiver-emitted payload.
    ///         Note that this is not a security critical field. It's meant to be used by messaging providers to
    /// identify which messages are Transceiver-related.
    ...
    function _initializeTransceiver() internal {
        ...
        wormhole.publishMessage{ value: msg.value }(0, TransceiverStructs.encodeTransceiverInit(init), consistencyLevel);
    }
    ...
    bytes32 private constant WORMHOLE_EVM_CHAIN_IDS = bytes32(uint256(keccak256("whTransceiver.evmChainIds")) - 1);
    ...
    function _getWormholeEvmChainIdsStorage() internal pure returns (mapping(uint16 => BooleanFlag) storage $) {
    ...
    function setWormholePeer(uint16 peerChainId, bytes32 peerContract) external payable onlyOwner {
        ...
        TransceiverStructs.TransceiverRegistration memory registration = TransceiverStructs.TransceiverRegistration({
...
// lib/example-native-token-transfers/evm/src/Transceiver/WormholeTransceiver/WormholeTransceiver.sol
...
contract WormholeTransceiver is IWormholeTransceiver, IWormholeReceiver, WormholeTransceiverState {
    ...
    function receiveMessage(bytes memory encodedMessage) external {
        ...
        (parsedTransceiverMessage, parsedNttManagerMessage) =
            TransceiverStructs.parseTransceiverAndNttManagerMessage(WH_TRANSCEIVER_PAYLOAD_PREFIX, payload);
        ...
        returns (uint256 nativePriceQuote)
    {
        ...
        WormholeTransceiverInstruction memory weIns = parseWormholeTransceiverInstruction(instruction.payload);
    ...
    function _sendMessage(
        ...
        override
    {
        (TransceiverStructs.TransceiverMessage memory transceiverMessage, bytes memory encodedTransceiverPayload) =
        ...
        if (!weIns.shouldSkipRelayerSend && _shouldRelayViaStandardRelaying(recipientChain)) {
            ...
            uint64 sequence =
                wormhole.publishMessage{ value: wormholeFee }(0, encodedTransceiverPayload, consistencyLevel);
            ...
            wormhole.publishMessage{ value: deliveryPayment }(0, encodedTransceiverPayload, consistencyLevel);
