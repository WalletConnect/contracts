...
// lib/example-native-token-transfers/evm/src/NttManager/TransceiverRegistry.sol
...
abstract contract TransceiverRegistry {
    ...
    function _setTransceiver(address transceiver) internal returns (uint8 index) {
        ...
        if (transceiverInfos[transceiver].registered) {
            ...
            transceiverInfos[transceiver] =
                TransceiverInfo({ registered: true, enabled: true, index: _numTransceivers.registered });
    ...
    function _checkTransceiverInvariants(address transceiver) private view {
        ...
        bool transceiverInEnabledBitmap = (_enabledTransceiverBitmap.bitmap & uint64(1 << transceiverInfo.index)) != 0;
...
// lib/example-native-token-transfers/evm/src/interfaces/INttToken.sol
...
// lib/example-native-token-transfers/evm/src/libraries/TransceiverHelpers.sol
...
function min_0(uint256 a, uint256 b) pure returns (uint256) {
    return a < b ? a : b;
}
...
// lib/example-native-token-transfers/evm/src/libraries/external/Initializable.sol
...
interface IWormholeRelayerSend is IWormholeRelayerBase {
    ...
    /**
     * @notice Publishes an instruction for the delivery provider at `deliveryProviderAddress`
     * to relay a payload and external messages specified by `messageKeys` to the address `targetAddress` on chain
     * `targetChain`
     * with gas limit `gasLimit` and `msg.value` equal to
     * receiverValue + (arbitrary amount that is paid for by paymentForExtraReceiverValue of this chain's wei) in
     * targetChain wei.
     ...
     * paymentForExtraReceiverValue
     *
     * Note: MessageKeys can specify wormhole messages (VaaKeys) or other types of messages (ex. USDC CCTP
     * attestations). Ensure the selected
...
interface IWormholeRelayerDelivery is IWormholeRelayerBase {
     ...
     *     where a cross chain refund is necessary, or if the default code path is used where no refund is requested
     * (NO_REFUND_REQUESTED)
...
library BytesParsing {
    ...
    function asBoolUnchecked(bytes memory encoded, uint256 offset) internal pure returns (bool, uint256) {
        (uint8 val, uint256 nextOffset) = asUint8(encoded, offset);
        if (val & 0xfe != 0) {
...
// lib/example-native-token-transfers/evm/src/libraries/PausableUpgradeable.sol
...
// lib/example-native-token-transfers/evm/src/libraries/TrimmedAmount.sol
...
using { gt as >, lt as <, sub as -, add as +, eq as ==, min_1, unwrap } for TrimmedAmount global;
...
function min_1(TrimmedAmount a, TrimmedAmount b) pure returns (TrimmedAmount) {
    checkDecimals(a, b);
...
// lib/example-native-token-transfers/evm/src/libraries/external/ContextUpgradeable.sol
...
// lib/example-native-token-transfers/evm/src/libraries/external/ReentrancyGuardUpgradeable.sol
...
// lib/example-native-token-transfers/evm/src/interfaces/IRateLimiterEvents.sol
...
// lib/example-native-token-transfers/evm/src/libraries/TransceiverStructs.sol
...
// lib/example-native-token-transfers/evm/src/libraries/external/OwnableUpgradeable.sol
...
// lib/example-native-token-transfers/evm/src/interfaces/IManagerBase.sol
...
// lib/example-native-token-transfers/evm/src/interfaces/IRateLimiter.sol
...
// lib/example-native-token-transfers/evm/src/interfaces/ITransceiver.sol
...
// lib/example-native-token-transfers/evm/src/interfaces/INttManager.sol
...
// lib/example-native-token-transfers/evm/src/libraries/PausableOwnable.sol
...
// lib/example-native-token-transfers/evm/src/libraries/Implementation.sol
...
// lib/example-native-token-transfers/evm/src/libraries/RateLimiter.sol
...
abstract contract RateLimiter is IRateLimiter, IRateLimiterEvents {
    ...
    bytes32 private constant OUTBOUND_LIMIT_PARAMS_SLOT = bytes32(uint256(keccak256("ntt.outboundLimitParams")) - 1);

    bytes32 private constant OUTBOUND_QUEUE_SLOT = bytes32(uint256(keccak256("ntt.outboundQueue")) - 1);

    bytes32 private constant INBOUND_LIMIT_PARAMS_SLOT = bytes32(uint256(keccak256("ntt.inboundLimitParams")) - 1);
    ...
    function _getOutboundQueueStorage() internal pure returns (mapping(uint64 => OutboundQueuedTransfer) storage $) {
    ...
    function _getInboundLimitParamsStorage() internal pure returns (mapping(uint16 => RateLimitParams) storage $) {
    ...
    function _getInboundQueueStorage() internal pure returns (mapping(bytes32 => InboundQueuedTransfer) storage $) {
    ...
    constructor(uint64 _rateLimitDuration, bool _skipRateLimiting) {
        if (_rateLimitDuration == 0 && !_skipRateLimiting || _rateLimitDuration != 0 && _skipRateLimiting) {
    ...
    function _setLimit(TrimmedAmount limit, RateLimitParams storage rateLimitParams) internal {
        ...
        if (oldLimit.isNull()) {
            ...
            rateLimitParams.currentCapacity = _calculateNewCurrentCapacity(limit, oldLimit, currentCapacity);
        }
        ...
        returns (TrimmedAmount capacity)
    {
        ...
        if (rateLimitDuration == 0) {
            return packTrimmedAmount(type(uint64).max, rateLimitParams.currentCapacity.getDecimals());
        }
        ...
        unchecked {
            ...
            uint256 result = min_0(calculatedCapacity, rateLimitParams.limit.getAmount());
            return packTrimmedAmount(SafeCast.toUint64(result), rateLimitParams.currentCapacity.getDecimals());
        }
    }
    ...
    function _calculateNewCurrentCapacity(
        ...
        internal
        pure
        returns (TrimmedAmount newCurrentCapacity)
    {
    ...
    function _consumeOutboundAmount(TrimmedAmount amount) internal {
        ...
        _consumeRateLimitAmount(amount, _getCurrentCapacity(getOutboundLimitParams()), _getOutboundLimitParamsStorage());
    }
    ...
    function _consumeInboundAmount(TrimmedAmount amount, uint16 chainId_) internal {
        ...
        _consumeRateLimitAmount(
            amount, _getCurrentCapacity(getInboundLimitParams(chainId_)), _getInboundLimitParamsStorage()[chainId_]
        );
    }
    ...
    {
        rateLimitParams.lastTxTimestamp = uint64(block.timestamp);
        rateLimitParams.currentCapacity = capacity.saturatingAdd(amount).min_1(rateLimitParams.limit);
    }

    function _isOutboundAmountRateLimited(TrimmedAmount amount) internal view returns (bool) {
        return
            rateLimitDuration != 0 ? _isAmountRateLimited(_getCurrentCapacity(getOutboundLimitParams()), amount) : false;
    }

    function _isInboundAmountRateLimited(TrimmedAmount amount, uint16 chainId_) internal view returns (bool) {
        return rateLimitDuration != 0
            ? _isAmountRateLimited(_getCurrentCapacity(getInboundLimitParams(chainId_)), amount)
            : false;
    }

    function _isAmountRateLimited(TrimmedAmount capacity, TrimmedAmount amount) internal pure returns (bool) {
    ...
    function _enqueueInboundTransfer(bytes32 digest, TrimmedAmount amount, address recipient) internal {
        _getInboundQueueStorage()[digest] =
            InboundQueuedTransfer({ amount: amount, recipient: recipient, txTimestamp: uint64(block.timestamp) });
...
// lib/example-native-token-transfers/evm/src/NttManager/ManagerBase.sol
    ...
    Implementation
{
        ...
        returns (uint256[] memory, uint256)
    {
        ...
        TransceiverStructs.TransceiverInstruction[] memory instructions =
            TransceiverStructs.parseTransceiverInstructions(transceiverInstructions, enabledTransceivers.length);
    ...
    function _recordTransceiverAttestation(
        ...
        returns (bytes32)
    {
        bytes32 nttManagerMessageHash = TransceiverStructs.nttManagerMessageDigest(sourceChainId, payload);
        ...
        if (transceiverAttestedToMessage(nttManagerMessageHash, _getTransceiverInfosStorage()[msg.sender].index)) {
    ...
    function _prepareForTransfer(
        ...
        internal
        returns (address[] memory, TransceiverStructs.TransceiverInstruction[] memory, uint256[] memory, uint256)
    {
...

        {
            ...
            instructions =
                TransceiverStructs.parseTransceiverInstructions(transceiverInstructions, numRegisteredTransceivers);
        }
    ...
    function transceiverAttestedToMessage(bytes32 digest, uint8 index) public view returns (bool) {
        return _getMessageAttestationsStorage()[digest].attestedTransceivers & uint64(1 << index) > 0;
    }
    ...
    function _setTransceiverAttestedToMessage(bytes32 digest, address transceiver) internal {
        ...
        emit MessageAttestedTo(digest, transceiver, _getTransceiverInfosStorage()[transceiver].index);
    }
    ...
    function _getMessageAttestations(bytes32 digest) internal view returns (uint64) {
        ...
        return _getMessageAttestationsStorage()[digest].attestedTransceivers & enabledTransceiverBitmap;
    }

    function _getEnabledTransceiverAttestedToMessage(bytes32 digest, uint8 index) internal view returns (bool) {
...
// lib/example-native-token-transfers/evm/src/NttManager/NttManager.sol
...
contract NttManager is INttManager, RateLimiter, ManagerBase {
    ...
    /// @inheritdoc INttManager
    function setPeer(uint16 peerChainId, bytes32 peerContract, uint8 decimals, uint256 inboundLimit) public onlyOwner {
        if (peerChainId == 0) {
        ...
        emit PeerUpdated(peerChainId, oldPeer.peerAddress, oldPeer.tokenDecimals, peerContract, decimals);
    }
    ...
    {
        return
            _transferEntryPoint(amount, recipientChain, recipient, refundAddress, shouldQueue, transceiverInstructions);
    }
    ...
    function executeMsg(
        ...
        whenNotPaused
    {
        (bytes32 digest, bool alreadyExecuted) = _isMessageExecuted(sourceChainId, sourceNttManagerAddress, message);
    ...
    /// @inheritdoc INttManager
    function cancelOutboundQueuedTransfer(uint64 messageSequence) external nonReentrant whenNotPaused {
        // find the message in the queue
        OutboundQueuedTransfer memory queuedTransfer = _getOutboundQueueStorage()[messageSequence];
        if (queuedTransfer.txTimestamp == 0) {
            revert OutboundQueuedTransferNotFound(messageSequence);
        ...
        _mintOrUnlockToRecipient(bytes32(uint256(messageSequence)), msg.sender, queuedTransfer.amount, true);
    }
        ...
        returns (uint64)
    {
        ...
        return _transfer(
            sequence, trimmedAmount, recipientChain, recipient, refundAddress, msg.sender, transceiverInstructions
        );
    }
        ...
        returns (uint64 msgSequence)
    {
        ...
        TransceiverStructs.NativeTokenTransfer memory ntt =
            TransceiverStructs.NativeTokenTransfer(amount, toWormholeFormat(token), recipient, recipientChain);
        ...
        bytes memory encodedNttManagerPayload = TransceiverStructs.encodeNttManagerMessage(
            TransceiverStructs.NttManagerMessage(
                bytes32(uint256(seq)), toWormholeFormat(sender), TransceiverStructs.encodeNativeTokenTransfer(ntt)
            )
        );
        ...
        emit TransferSent(recipient, refundAddress, amt.untrim(tokenDecimals()), totalPriceQuote, destinationChain, seq);
    ...
    function tokenDecimals() public view override(INttManager, RateLimiter) returns (uint8) {
        (bool success, bytes memory queriedDecimals) = token.staticcall(abi.encodeWithSignature("decimals()"));
    ...
    function _trimTransferAmount(uint256 amount, uint16 toChain) internal view returns (TrimmedAmount) {
