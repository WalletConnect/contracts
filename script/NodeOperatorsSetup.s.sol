...
contract NodeOperatorsSetup is BaseScript {
    /**
     * @dev This script mints CNCT tokens for the node operators. It is useful for setting up
    ...
    function run() external {
        ...
        for (uint256 i; i < nodeOperatorsLength; i++) {
            console2.log("Minting CNCT for ", vm.getLabel(nodeOperators[i]), " at address ", nodeOperators[i]);
            deployments.cnct.mint(nodeOperators[i], defaults.MIN_STAKE());
