...
contract ClaimRewards_Staking_Integration_Concrete_Test is Staking_Integration_Shared_Test {
        ...
        givenCallerHasPendingRewards(users.permissionedNode)
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                address(staking),
                cnct.allowance(address(staking), users.treasury),
    ...
    modifier givenStakingHasAllowanceFromRewardsVault() {
        ...
        cnct.approve(address(staking), defaults.EPOCH_REWARD_EMISSION());
        vm.stopPrank();
        _;
    }
        ...
        givenStakingHasAllowanceFromRewardsVault
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                cnct.balanceOf(address(users.treasury)),
                defaults.EPOCH_REWARD_EMISSION()
            )
        );
        staking.claimRewards();
        ...
        givenStakingHasAllowanceFromRewardsVault
    {
        ...
        cnct.mint(address(users.treasury), defaults.EPOCH_REWARD_EMISSION());
        vm.stopPrank();
        uint256 initialPendingRewards = staking.pendingRewards(users.permissionedNode);
        uint256 initialTreasuryBalance = cnct.balanceOf(address(users.treasury));
        ...
        uint256 finalTreasuryBalance = cnct.balanceOf(address(users.treasury));
