...
contract Stake_Staking_Integration_Concrete_Test is Staking_Integration_Shared_Test {
        ...
        whenAmountToStakeGTEMinStake
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                address(staking),
                cnct.allowance(address(staking), users.permissionedNode),
                amount
            )
        );
        staking.stake(amount);
    ...
    modifier whenCallerHasApprovedStakingAmount(address caller) {
        ...
        cnct.approve(address(staking), defaults.MIN_STAKE());
        vm.stopPrank();
        _;
    }
        ...
        whenCallerHasApprovedStakingAmount(users.permissionedNode)
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                cnct.balanceOf(users.permissionedNode),
                amount
            )
        );
        ...
        whenCallerHasApprovedStakingAmount(users.permissionedNode)
    {
        ...
        cnct.mint(users.permissionedNode, defaults.MIN_STAKE());
        vm.stopPrank();
        uint256 initialStake = staking.stakes(users.permissionedNode);
        uint256 initialStakingBalance = cnct.balanceOf(address(staking));
        uint256 initialNodeBalance = cnct.balanceOf(users.permissionedNode);
        ...
        vm.assertEq(cnct.balanceOf(address(staking)), initialStakingBalance + defaults.MIN_STAKE());
        // it should decrease the balance of the caller by the amount of tokens staked
        vm.assertEq(cnct.balanceOf(users.permissionedNode), initialNodeBalance - defaults.MIN_STAKE());
    }
        ...
        whenCallerIsNotAPermissionedNode
    {
        ...
        cnct.mint(users.nonPermissionedNode, defaults.MIN_STAKE());
        vm.stopPrank();
        uint256 initialStake = staking.stakes(users.nonPermissionedNode);
        uint256 initialStakingBalance = cnct.balanceOf(address(staking));
        uint256 initialNodeBalance = cnct.balanceOf(users.nonPermissionedNode);
        ...
        vm.assertEq(cnct.balanceOf(address(staking)), initialStakingBalance + defaults.MIN_STAKE());
        // it should decrease the balance of the caller by the amount of tokens staked
        vm.assertEq(cnct.balanceOf(users.nonPermissionedNode), initialNodeBalance - defaults.MIN_STAKE());
    }
}
