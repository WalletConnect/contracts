...
contract RevokeRole_L2CNKT_Unit_Concrete_Test is Base_Test {
    ...
    function setUp() public override {
        ...
        DEFAULT_ADMIN_ROLE = l2cnkt.DEFAULT_ADMIN_ROLE();
        MANAGER_ROLE = l2cnkt.MANAGER_ROLE();
    }
    ...
    function test_RevertWhen_CallerNotDefaultAdmin() external whenRevokingDefaultAdminRole {
        ...
        l2cnkt.revokeRole(DEFAULT_ADMIN_ROLE, users.admin);
    }
    ...
    function test_RevertWhen_AttackerNotDefaultAdmin() external whenRevokingManagerRole {
        ...
        l2cnkt.revokeRole(MANAGER_ROLE, users.manager);
    }
    ...
    function test_WhenRevokingDefaultAdminRole() external whenCallerDefaultAdmin whenRevokingDefaultAdminRole {
        assertTrue(l2cnkt.hasRole(DEFAULT_ADMIN_ROLE, users.admin));
        vm.expectEmit(true, true, true, true);
        emit RoleRevoked(DEFAULT_ADMIN_ROLE, users.admin, users.admin);
        l2cnkt.revokeRole(DEFAULT_ADMIN_ROLE, users.admin);
        assertFalse(l2cnkt.hasRole(DEFAULT_ADMIN_ROLE, users.admin));
    }
    ...
    {
        assertTrue(!l2cnkt.hasRole(DEFAULT_ADMIN_ROLE, users.attacker));
        l2cnkt.revokeRole(DEFAULT_ADMIN_ROLE, users.attacker);
    ...
    function test_WhenRevokingManagerRole() external whenCallerDefaultAdmin whenRevokingManagerRole {
        assertTrue(l2cnkt.hasRole(MANAGER_ROLE, users.manager));
        vm.expectEmit(true, true, true, true);
        emit RoleRevoked(MANAGER_ROLE, users.manager, users.admin);
        l2cnkt.revokeRole(MANAGER_ROLE, users.manager);
        assertFalse(l2cnkt.hasRole(MANAGER_ROLE, users.manager));
    }

    function test_WhenRevokingManagerRoleFromNonHolder() external whenCallerDefaultAdmin whenRevokingManagerRole {
        assertTrue(!l2cnkt.hasRole(MANAGER_ROLE, users.attacker));
        l2cnkt.revokeRole(MANAGER_ROLE, users.attacker);
        // No state change, but should not revert
    }
}
