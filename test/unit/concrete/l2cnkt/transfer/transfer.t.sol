...
contract Transfer_L2CNKT_Unit_Concrete_Test is Base_Test {
    ...
    {
        uint256 initialBalanceAlice = l2cnkt.balanceOf(users.alice);
        uint256 initialBalanceBob = l2cnkt.balanceOf(users.bob);

        vm.prank(users.alice);
        vm.expectEmit(true, true, true, true);
        emit Transfer(users.alice, users.bob, 100);

        bool success = l2cnkt.transfer(users.bob, 100);

        assertTrue(success);
        assertEq(l2cnkt.balanceOf(users.alice), initialBalanceAlice - 100);
        assertEq(l2cnkt.balanceOf(users.bob), initialBalanceBob + 100);
    }
        ...
        givenSenderHasInsufficientBalance
    {
        ...
        l2cnkt.transfer(users.bob, 1001);
    }
