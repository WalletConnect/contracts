...
contract Merkle {
    ...
    function getRoot(bytes32[] memory data) public pure returns (bytes32) {
        require(data.length > 1, "won't generate root for zero leafs");
        // if(data.length == 1) {
        //     return data[0];
        // }
        while (data.length > 1) {
    ...
    function getProof(bytes32[] memory data, uint256 node) public pure returns (bytes32[] memory) {
        require(data.length > 1, "won't generate proof for zero leafs");
        // if(data.length == 1){
        //     return data;
        // }
    ...
    function hashLevel(bytes32[] memory data) private pure returns (bytes32[] memory) {
        ...
        // Modified to push up the last leaf instead of hashing it with a 0 leaf.

        unchecked {
            uint256 length = data.length;
            bool isLengthOdd = length & 0x1 == 1;
            uint256 resultLength = (length + 1) / 2; // This handles both even and odd lengths correctly
            ...
            // If the length is odd, push up the last leaf
            if (isLengthOdd) {
                result[pos] = data[length - 1];
            }
        }
        return result;
    }
